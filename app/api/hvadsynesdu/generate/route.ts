...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const sourceMode = body.sourceMode === "current-source" ? "current-source" : "original";
    ...
    const content = await createStructuredResponse<GeneratedContent>({
      apiKey,
      prompt: contentPrompt({ level: body.level, targetLevel: body.targetLevel, sourceMode, recentTopics }),
      schemaName: "danish_learning_session",
      schema: CONTENT_SCHEMA,
      signal: request.signal,
      useWebSearch: sourceMode === "current-source",
